...
const DELAY = 250;
const CONCURRENCY = 25; // 25 parallel requests! 🔥💀
const LIMIT = process.argv.includes("--limit")
    ? parseInt(process.argv[process.argv.indexOf("--limit") + 1])
    : null;
const START_FROM = process.argv.includes("--start")
    ? parseInt(process.argv[process.argv.indexOf("--start") + 1])
    : null;
...
function extractMetadata(html: string, objectId: number) {
    ...
    try {
        ...
        const iiifMatch = html.match(
            /<meta property="og:image" content="([^"]*iiif[^"]*)"[^>]*>/i
        );
        if (iiifMatch) {
        ...
        const descMatch = html.match(
            /<div[^>]*class="[^"]*artwork__intro__desc[^"]*"[^>]*itemprop="description"[^>]*>\s*<p[^>]*>([^<]+(?:<br[^>]*>[^<]*)*)<\/p>/s
        );
        if (descMatch) {
        ...
        if (jsonLdMatch) {
            try {
                ...
                if (jsonData.creator && jsonData.creator.name)
                    data.artist = jsonData.creator.name;
            } catch (e) {
...
async function processArtwork(
    ...
    try {
        ...
        console.log(
            `[${stats.processed}] Processing: "${artworkItem.title}" (${artworkItem.object_id})`
        );
        ...
        const foundImage =
            metadata.iiif_image_url && !artworkItem.current_primary_image;
        const foundDescription =
            metadata.description && !artworkItem.current_description;
        const foundEnhancedData =
            metadata.artist || metadata.date || metadata.culture;

        if (foundImage || foundDescription || foundEnhancedData) {
            console.log(`   ✅ Found useful data!`);
            if (foundImage) {
                console.log(
                    `      🖼️  New IIIF image: ${metadata.iiif_image_url}`
                );
                stats.newImages++;
            }
            if (foundDescription) {
            ...
            if (foundEnhancedData && !foundImage && !foundDescription) {
                console.log(
                    `      📊 Enhanced: artist=${!!metadata.artist}, date=${!!metadata.date}, culture=${!!metadata.culture}`
                );
            }
            ...
            if (!DRY_RUN) {
                ...
                if (
                    metadata.iiif_image_url &&
                    !artworkItem.current_primary_image
                ) {
                ...
                if (metadata.credit_line)
                    updateData.credit_line = metadata.credit_line;
                if (metadata.classification)
                    updateData.classification = metadata.classification;
                if (metadata.artist_bio)
                    updateData.artist_display_bio = metadata.artist_bio;
...
async function main() {
    ...
    if (START_FROM) {
        console.log(
            `🎯 Starting from object_id: ${START_FROM} (skip already processed)`
        );
    }
    console.log("");
    ...
    // Add starting point if specified
    if (START_FROM) {
        const { and, lte } = await import("drizzle-orm");
        whereCondition = and(
            whereCondition,
            lte(artwork.object_id, START_FROM)
        );
    }
    ...
    console.log(
        `   ⚡ Estimated time: ~${Math.ceil(
            (artworksToScrape.length * (DELAY / 1000)) / CONCURRENCY / 3600
        )} hours\n`
    );
    ...
    const progressInterval = setInterval(() => {
        ...
        const eta = Math.ceil(
            (artworksToScrape.length - stats.processed) / rate / 3600
        );
        ...
        console.log(
            `⚡ Rate: ${rate.toFixed(1)}/sec | ETA: ${
                eta > 0 ? eta + "h" : "Almost done!"
            }\n`
        );
    ...
    console.log(
        `⚡ Average rate: ${(stats.processed / totalTime).toFixed(1)} items/sec`
    );
    console.log(
